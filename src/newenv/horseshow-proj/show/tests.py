...
class BillTests(TestCase):
    def test_billpage_setup(self):
        ...
        rider1 = Rider.objects.create(last_name="Bob", address="555 ct", birth_date="1990-09-25",
                                      email="55@s.edu", member_VHSA=True, county="fairfax")
        c1 = Class.objects.create(last_name="Test", num="1")
    ...
    def test_billpage_pricecheck(self):
        horse1 = Horse.objects.create(last_name="Lollipop", coggins_date="2011-10-11",
                                      accession_num=48, owner="John", size="medium", type="horse")
        rider1 = Rider.objects.create(last_name="Bob", address="555 ct", birth_date="1990-09-25",
                                      email="55@s.edu", member_VHSA=True, county="fairfax")
        c2 = Class.objects.create(last_name="Test2", num="2")
        combo = HorseRiderCombo.objects.create(
            num=555, rider=rider1, horse=horse1)
        class_participation = ClassParticipation(
...
class CheckAge(TestCase):
    def test_calculate_age(self):
        rider = Rider.objects.create(last_name="Anna Wu", address="address1", city="cville", state="VA",
    ...
    def test_calculate_age2(self):
        rider1 = Rider.objects.create(last_name="Ashley Ontiri", address="address2", city="princeton", state="NJ", zip_code="2290310",
                                      email="aw@email.com", adult=False, birth_date=datetime.datetime.strptime('2010113', "%Y%m%d").date())
        horse1 = Horse.objects.create(last_name="Ruby", coggins_date=datetime.datetime.strptime(
        ...
        if age <= 14:
            self.assertTrue(combo1.rider.last_name == "Ashley Ontiri")
...
class CheckHorseType(TestCase):
    def test_horse_type_check(self):
        ...
        horse1 = Horse.objects.create(last_name="Ruby", coggins_date=datetime.datetime.strptime(
            '20100522', "%Y%m%d").date(), accession_num="ace123", owner="Anna Wu", type="horse", size="NA")
        horse2 = Horse.objects.create(last_name="Watchme", coggins_date=datetime.datetime.strptime(
            '20090811', "%Y%m%d").date(), accession_num="ace321", owner="Angie Lee", type="pony", size="small")
        horse3 = Horse.objects.create(last_name="Strange", coggins_date=datetime.datetime.strptime(
            '20110524', "%Y%m%d").date(), accession_num="ace567", owner="Sarah Chu", type="pony", size="medium")
        rider1 = Rider.objects.create(last_name="Anna Wu", address="address1", city="cville", state="VA",
                                      zip_code="22903", email="aw@email.com", adult=False, birth_date=datetime.datetime.strptime('20040122', "%Y%m%d").date())
        rider2 = Rider.objects.create(last_name="Ashley Ontiri", address="address2", city="princeton", state="NJ", zip_code="08541",
                                      email="ao@email.com", adult=True)
        rider3 = Rider.objects.create(last_name="Anne Katherine", address="address3", city="vienna", state="VA", zip_code="22181",
        ...
        if combo1.horse.type == "pony":
            list.append(combo1.rider.last_name)
        if combo2.horse.type == "pony":
            list.append(combo2.rider.last_name)
        if combo3.horse.type == "pony":
            list.append(combo3.rider.last_name)
...
class ViewsTestCases(TestCase):
    ...
    def test_add_show_get(self):
        show = Show.objects.create(
            last_name="test", date="2018-12-10", location="here", day_of_price=10, pre_reg_price=5)
        new_division = Division.objects.create(last_name="division")
        request = HttpRequest()
        client = Client()
        response = client.get(reverse('add_show'))

    def test_add_show_post(self):
        self.client.login(userlast_name='john', password='johnpassword')
        request = HttpRequest()
        response = self.client.post(
            '/show/add', {'last_name': 'test', 'date': '2018-12-10', 'location': 'here', 'day_of_price': 10, 'pre_reg_price': 5})
...
class CheckEntryNum(TestCase):
    def test_entry_num(self):
        ...
        show = Show.objects.create(date=datetime.datetime.strptime('20190526', "%Y%m%d"), last_name="Show1",
                                   location="Cville", day_of_price=15, pre_reg_price=11)
        div = Division.objects.create(id=1, last_name="Div1", show=show)
        c1 = Class.objects.create(
            num=1, last_name="Class1", division=div, show=show)
...
class CheckRider(TestCase):
    def setUp(self):
        ...
        self.show = Show.objects.create(
            date='2019-10-12', pre_reg_price=5, day_of_price=2, last_name="5th annual", location="some_place")

        self.rider1 = Rider.objects.create(last_name="Bob", address="555 ct", birth_date="1990-09-25",
                                           email="55@s.edu", member_VHSA=True, county="fairfax")

        self.rider2 = Rider.objects.create(last_name="Ashley Ontiri", address="address2", city="princeton", zip_code="22903",

...
def write_fillable_pdf(input_pdf_path, output_pdf_path, data_dict):
    ...
    for i in range(1, 16):
        annotations = template_pdf.pages[i][ANNOT_KEY] #populating data onto the second page of the pdf 
...
def read_pdf(input_pdf_path, page, key_index):
    template_pdf = pdfrw.PdfReader(input_pdf_path) #reads the file from the input_pdf_path that was passed in
    annotations = template_pdf.pages[page-1][ANNOT_KEY] #populating data onto the second page of the pdf 
    # for annotation in annotations:
        # print(annotation)
